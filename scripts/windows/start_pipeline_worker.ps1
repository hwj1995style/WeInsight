...
try {
    ...
    $modulePath = Join-Path $resolvedProjectRoot "app\workers\pipeline_main.py"
    if (-not (Test-Path -LiteralPath $modulePath -PathType Leaf)) {
        throw [System.IO.FileNotFoundException]::new("Pipeline module is missing.")
    }

    $logDir = Join-Path $resolvedProjectRoot "runtime\logs\pipeline_worker"
    [IO.Directory]::CreateDirectory($logDir) | Out-Null
    $logFile = Join-Path $logDir (
        "pipeline_worker_{0}.log" -f (Get-Date -Format "yyyyMMdd")
    )

    $mysqlPassword = Resolve-OptionalEnv -Name "WEINSIGHT_PIPELINE_MYSQL_PASSWORD"
    ...
    $arguments = @("-m", "app.workers.pipeline_main", "--config", $resolvedConfigPath)
    Write-SafeLog -Path $logFile -Message (
        "[{0}] process_start process=pipeline_worker module=app.workers.pipeline_main config_path={1}" -f (
    ...
    Write-SafeLog -Path $logFile -Message (
        "[{0}] process_exit process=pipeline_worker {1}" -f (
            Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        ), $exitSummary
    )
    exit $exitCode
    ...
    if ($null -ne $logFile) {
        Write-SafeLog -Path $logFile -Message (
            "[{0}] process_failed process=pipeline_worker exception_type={1} exit_code={2}" -f (
                Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            ), $exceptionType, 1
        )
    }
    [Console]::Error.WriteLine(
        "startup_failed process=pipeline_worker exception_type={0}" -f $exceptionType
    )
    exit 1
}
finally {
