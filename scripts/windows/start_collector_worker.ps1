...
try {
    ...
    $modulePath = Join-Path $resolvedProjectRoot "app\workers\collector_main.py"
    if (-not (Test-Path -LiteralPath $modulePath -PathType Leaf)) {
        throw [System.IO.FileNotFoundException]::new("Collector module is missing.")
    }

    $logDir = Join-Path $resolvedProjectRoot "runtime\logs\collector_worker"
    [IO.Directory]::CreateDirectory($logDir) | Out-Null
    $logFile = Join-Path $logDir (
        "collector_worker_{0}.log" -f (Get-Date -Format "yyyyMMdd")
    )
    ...
    $arguments = @("-m", "app.workers.collector_main", "--config", $resolvedConfigPath)
    Write-SafeLog -Path $logFile -Message (
        "[{0}] process_start process=collector_worker module=app.workers.collector_main config_path={1}" -f (
    ...
    Write-SafeLog -Path $logFile -Message (
        "[{0}] process_exit process=collector_worker {1}" -f (
            Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        ), $exitSummary
    )
    exit $exitCode
    ...
    if ($null -ne $logFile) {
        Write-SafeLog -Path $logFile -Message (
            "[{0}] process_failed process=collector_worker exception_type={1} exit_code={2}" -f (
                Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            ), $exceptionType, 1
        )
    }
    [Console]::Error.WriteLine(
        "startup_failed process=collector_worker exception_type={0}" -f $exceptionType
    )
    exit 1
}
finally {
