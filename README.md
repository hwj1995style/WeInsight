...
> 当前边界：真实微信/RPA 验证仅适用于微信群。公众号旧 article-RPA 已退役，公众号现行采集路径仅为 WeRSS + WeInsight RSS。下文链接的公众号 article-RPA 历史文档只供审计，命令已移除且禁止执行；现行操作统一使用 `docs/operations/公众号RSS采集运行手册.md`。
...
## 第五阶段公众号/订阅号受控真实 POC（历史 article-RPA）
...
以下内容记录切换前的公众号 article-RPA 验证过程，仅供历史审计，禁止执行其中命令。公众号不得再回滚到手动 article-RPA；当前 POC、扩容与回滚必须使用 WeRSS + WeInsight RSS 运行手册。

公众号 article-RPA 历史文档（已退役，禁止执行）：
...
本阶段记录中的真实微信/RPA 操作仅对核心群有效。公众号部分是退役 article-RPA 历史记录，禁止执行；公众号现行 POC 使用 WeRSS + WeInsight RSS。AI 仍保持 dry-run，输出必须确认 `model_called=0`。
...
第九阶段的真实微信/RPA 验证仅适用于微信群。公众号旧 article-RPA 执行记录均为历史且禁止执行；公众号现行路径按 RSS 手册先做单公众号 24 小时 POC，通过后再扩到 3 个公众号。AI 继续 dry-run 且必须确认 `model_called=0`。
...
微信群真实微信/RPA 验证必须依次通过 Fake RPA、单目标人工值守和次日日报门禁。此处真实微信 POC 仅指微信群；公众号不再执行 RPA。未经人工批准不得执行真实微信 POC，也不得在记录中预填 Go 结论：
...
公众号页另有一条隔离的 WeRSS 授权管理链路：状态查询复用只读 AK/SK，但只对白名单字段解析且不会展示或保存 Token/Cookie。WeRSS 管理用户名/密码、SMTP 参数和多个收件人可在公众号页直接配置，两个密码使用当前 Windows 运行账号的 DPAPI 加密落库且永不回显；保存后 Web 与 Pipeline Worker 动态读取，无需重启。`WERSS_AUTH_ADMIN_PASSWORD` 与 `WERSS_AUTH_SMTP_PASSWORD` 仅作为数据库尚未保存密码时的回退。部署新增版本时，先执行 `sql/migrations/20260716_001_create_werss_authorization_management.sql`，再重启 Web 与 Pipeline Worker；两进程必须由同一 Windows 账号运行才能解密密码。
