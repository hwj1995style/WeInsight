...
后续独立复审发现并关闭一个 Minor：Python `ip_address` 会把整数和四字节值解释为 IP。生产 host 现在在解析前严格要求非空、trim、无 Unicode `C*` 字符的字符串，整数和 bytes 无法再绕过绑定地址类型契约。复审未发现新的 Critical、Important 或 Minor，Assessment 为 Ready。
