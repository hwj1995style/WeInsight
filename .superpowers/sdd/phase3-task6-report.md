...
Task 6 独立审查发现并修复三组恢复边界。先补测试并观察到 10 个预期 RED，分别命中缺少实时 checkpoint 时钟、托管工厂未注入核心群 due provider、四类 claim 后异常仍逃逸，以及 heartbeat 写入被 stopping/stopped 超车。

- 托管运行时现在使用同一 Engine 构造 `MysqlGroupConfigRepo` 和 `ReadOnlyCoreGroupDueProvider`，并向 provider 与 Article Runner 注入同一个上海时区 clock。三个文章 checkpoint 每次重新取当前时间，并对显式注入值执行严格 `Asia/Shanghai` ZoneInfo 校验；未注入 clock 的旧调用继续沿用 `run_once(now)`，保持 naive 兼容。OPEN_ACCOUNT、COPY_LINKS、SAVE_LINKS 抢占和长运行中时间前进均有测试固定。
- Worker 新增 heartbeat 写互斥，原子注册、后台 heartbeat、`mark_stopping` 和 `mark_stopped` 使用同一写锁。确定性阻塞竞态验证最终写序为 running → stopping → stopped，不会在 stopped 后回写旧状态；调度器仍按 `shutdown(wait=False)` 收束。
- claim 成功后，claimed event 和 `execute_run` 整体进入瞬时故障边界。Event、start_target、finish_target 或 finish_run 异常会安全脱敏并标记 degraded，返回携带原 run id/pipeline 的 degraded tick，不退出主循环、不重跑该 run/RPA，也不伪造 target/run 完成。后台 heartbeat 和过期租约恢复包装仍可执行，主循环继续指数退避。

## 最终验证

- 独立审查新增聚焦测试：`11 passed`；Article/Worker 相关回归：`75 passed`。
- Managed Worker 专项：`53 passed`。
- Task 6 + Runner/Config/Task 4/Task 5 组合回归：`225 passed`。
- 全量测试：`1266 passed`。
