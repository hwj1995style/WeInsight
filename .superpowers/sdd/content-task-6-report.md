...
- 固定 POC：未启动（阻断中）。WeRSS 的 `feeds` 表已存在“湖南三尖农牧公司”Feed ID `MP_WXS_3545051769`；缺少的是 WeInsight 账号配置。未切换 `werss_first`，未扩大范围。
...
- WeRSS 数据库已有 9 个 Feed。名称有一处待确认：需求“江西九江祺壳蛋”，WeRSS 实际“江西九江褐壳蛋”。
- 湖南真实 Feed 核验：HTTP 200、25 条；现有 `RssFeedClient` locator 提取数为 0，条目未含当前识别的扩展字段。已新增 locator 兼容实现任务；因此湖南正文 POC 仍未启动，不宣称 24 小时通过。
